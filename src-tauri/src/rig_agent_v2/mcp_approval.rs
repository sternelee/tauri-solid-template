...
impl McpApprovalConfig {
    ...
    pub fn default() -> Self {
        ...
        // 添加默认的mcp-hub配置
        mcp_servers.insert("mcp-hub".to_string(), McpServerConfig {
            auto_approve: vec![
    ...
    /// 保存到配置文件
    pub async fn save_to_file(&self, config_path: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    ...
    /// 切换工具的自动批准状态
    pub fn toggle_tool_auto_approve(&mut self, server_name: &str, tool_name: &str) -> Result<bool, String> {
        if let Some(config) = self.mcp_servers.get_mut(server_name) {
