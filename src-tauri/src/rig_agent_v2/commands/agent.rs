...
pub async fn initialize_agent_system(
    ...
    match crate::rig_agent_v2::AgentManager::new(config).await {
        Ok(manager) => {
        ...
        Err(e) => {
            Err(format!("Failed to initialize agent system: {}", e))
        }
    }
}
...
pub async fn get_agent_info(app: AppHandle) -> std::result::Result<serde_json::Value, String> {
    ...
    match app.try_state::<std::sync::Arc<crate::rig_agent_v2::AgentManager>>() {
        Some(manager) => {
        ...
        None => {
            Err("Agent system not initialized".to_string())
        }
    }
}
