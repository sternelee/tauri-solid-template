...
impl EventEmitter {
    ...
    pub async fn subscribe(&self, subscription: EventSubscription) -> broadcast::Receiver<AgentEvent> {
    ...
    /// Get events by type
    pub async fn get_events_by_type(&self, event_type: &EventType, limit: Option<usize>) -> Vec<AgentEvent> {
        let history = self.event_history.read().await;
    ...
    pub async fn get_events_by_session(&self, session_id: &str, limit: Option<usize>) -> Vec<AgentEvent> {
        let history = self.event_history.read().await;
        let mut filtered_events: Vec<AgentEvent> = history
            .iter()
            .filter(|event| event.session_id.as_ref().map_or(false, |id| id == session_id))
            .cloned()
            .collect();
    ...
    pub async fn get_events_by_conversation(&self, conversation_id: &str, limit: Option<usize>) -> Vec<AgentEvent> {
        let history = self.event_history.read().await;
        let mut filtered_events: Vec<AgentEvent> = history
            .iter()
            .filter(|event| event.conversation_id.as_ref().map_or(false, |id| id == conversation_id))
            .cloned()
            .collect();
    ...
    pub async fn get_events_by_severity(&self, severity: &EventSeverity, limit: Option<usize>) -> Vec<AgentEvent> {
        let history = self.event_history.read().await;
        let mut filtered_events: Vec<AgentEvent> = history
            .iter()
    ...
    pub async fn export_events(&self, format: ExportFormat) -> Result<String, EventError> {
        ...
        match format {
            ...
            ExportFormat::Csv => {
                self.export_events_to_csv(&history)
            }
        }
    }
...
impl EventEmitter {
    ...
    pub fn global() -> Arc<Self> {
        GLOBAL_EMITTER.get_or_init(|| Arc::new(Self::with_default_buffer())).clone()
    }
    ...
    pub async fn subscribe_global(subscription: EventSubscription) -> broadcast::Receiver<AgentEvent> {
