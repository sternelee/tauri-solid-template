...
use crate::rig_agent::mcp_approval_manager::McpApprovalManager;
use serde_json::Value;
use std::sync::Arc;

/// MCP工具适配器，用于将MCP工具集成到rig_agent
...
/// 为rig_agent的ToolManager提供的MCP工具函数
...
pub async fn refresh_mcp_tools(
    _tool_manager: &crate::rig_agent::core::types::ToolManager,
    mcp_state: Arc<McpState>,
) -> Result<(), String> {
    // 检查MCP服务器状态
...
pub async fn refresh_and_register_mcp_tools(
    tool_manager: &mut crate::rig_agent::core::types::ToolManager,
    mcp_state: Arc<McpState>,
) -> Result<usize, String> {
    let adapter = McpToolAdapter::new(mcp_state.clone());
