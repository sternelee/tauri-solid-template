...
impl AgentEvent {
    ...
    pub fn error(title: String, description: String, error: EventError) -> Self {
        Self::new(EventType::Error, title, description).with_error(error)
    }

    /// Convenience method for warning events
    pub fn warning(title: String, description: String) -> Self {
        Self::new(EventType::Warning, title, description).with_severity(EventSeverity::Warning)
    }
    ...
    pub fn debug(title: String, description: String) -> Self {
        Self::new(EventType::Debug, title, description).with_severity(EventSeverity::Debug)
    }
    ...
    pub fn tool_event(
        ...
        Self::new(event_type, title, description).with_source(EventSource::Tool { tool_name })
    }
    ...
    pub fn matches_subscription(&self, subscription: &super::EventSubscription) -> bool {
        ...
        if !subscription.event_types.is_empty()
            && !subscription.event_types.contains(&self.event_type)
        {
            return false;
        }
