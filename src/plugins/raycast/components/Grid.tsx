...
export function Grid(props: GridProps): JSX.Element {
  ...
  const getGridClasses = () => {
    const baseClasses =
      "raycast-grid grid gap-4 p-4 bg-white dark:bg-gray-900 overflow-y-auto";
  ...
  return (
    <div class="raycast-grid-container flex h-full flex-col bg-white dark:bg-gray-900">
      {/* Search Bar */}
      <Show when={props.searchBarAccessory || props.onSearchTextChange}>
        <div class="raycast-grid-search-bar flex items-center border-b border-gray-200 p-4 dark:border-gray-700">
      ...
      <Show when={props.navigationTitle}>
        <div class="raycast-grid-header border-b border-gray-200 px-4 py-3 dark:border-gray-700">
...
export function GridItem(props: GridItemProps): JSX.Element {
  ...
  return (
    <div
      class={`raycast-grid-item focus:ring-opacity-50 cursor-pointer rounded-lg border border-gray-200 bg-white p-4 transition-all duration-200 hover:border-gray-300 hover:shadow-md focus:ring-2 focus:ring-blue-500 focus:outline-none dark:border-gray-700 dark:bg-gray-800 dark:hover:border-gray-600 ${isSelected() ? "ring-opacity-50 bg-blue-50 ring-2 ring-blue-500 dark:bg-blue-900/20" : ""} ${isHovered() ? "-translate-y-1 transform" : ""} `}
      onClick={handleClick}
    ...
    >
      ...
      <div class="raycast-grid-item-text text-center">
        <Show when={props.title}>
          <h3 class="mb-1 truncate text-sm font-medium text-gray-900 dark:text-gray-100">
            {props.title}
          </h3>
        </Show>
      ...
      <Show when={props.actions && isHovered()}>
        <div class="raycast-grid-item-actions absolute top-2 right-2 opacity-0 transition-opacity group-hover:opacity-100">
          {props.actions}
        </div>
      </Show>
    </div>
...
export function GridSection(props: GridSectionProps): JSX.Element {
  return (
    <div class="raycast-grid-section col-span-full">
      ...
      {/* Section Content */}
      <div class="raycast-grid-section-content col-span-full grid grid-cols-subgrid gap-4">
        {props.children}
      </div>
    </div>
  );
...
export function GridEmptyView(props: GridEmptyViewProps): JSX.Element {
  return (
    <div class="raycast-grid-empty flex h-full flex-col items-center justify-center p-8 text-center">
      <Show when={props.icon}>
        <div class="raycast-grid-empty-icon mb-6">
          <ImageComponent image={props.icon!} class="h-20 w-20 text-gray-400" />
        </div>
      </Show>

      <Show when={props.title}>
        <h3 class="raycast-grid-empty-title mb-3 text-xl font-medium text-gray-900 dark:text-gray-100">
          {props.title}
        </h3>
      </Show>

      <Show when={props.description}>
        <p class="raycast-grid-empty-description mb-6 max-w-md text-sm leading-relaxed text-gray-500 dark:text-gray-400">
          {props.description}
        </p>
      </Show>

      <Show when={props.actions}>
        <div class="raycast-grid-empty-actions">{props.actions}</div>
      </Show>
    </div>
  );
}
...
function ContentRenderer(props: { content: any }): JSX.Element {
  ...
  if (typeof props.content === "string") {
    ...
    if (isImageUrl(props.content)) {
      return (
        <img
          src={props.content}
          alt=""
          class="raycast-grid-content-image h-24 w-full rounded-md object-cover"
        />
      );
    }
    ...
    if (isEmoji(props.content)) {
      return (
        <span class="raycast-grid-content-emoji text-4xl">{props.content}</span>
      );
    }
  ...
  if (typeof props.content === "object" && props.content !== null) {
    return (
      <ImageComponent
        image={props.content as ImageLike}
...
export function GridWithSidebar(
  props: GridProps & { sidebarContent?: JSX.Element; sidebarWidth?: number },
): JSX.Element {
  ...
  return (
    <div class="raycast-grid-with-sidebar flex h-full">
      ...
      <Show when={props.sidebarContent}>
        <div
          class="raycast-grid-sidebar flex-shrink-0 overflow-y-auto border-l border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800"
          style={{ width: `${sidebarWidth}px` }}
        >
          <div class="p-4">{props.sidebarContent}</div>
        </div>
      </Show>
    </div>
  );
