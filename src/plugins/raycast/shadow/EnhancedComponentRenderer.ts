...
export class EnhancedComponentRenderer implements ComponentRenderer {
  ...
  createBaseComponent(solidComponent: any, raycastProps: any): any {
    // Apply theme styling
    const themedProps = this.applyRaycastStyling(raycastProps, this.getCurrentTheme());
  ...
  private createComponentElement(type: string, props: any, children?: any[]): any {
  ...
  private createErrorComponent(type: string, error: any): any {
    return this.createComponentElement('div', {
      class: 'raycast-error-component bg-red-50 border border-red-200 rounded-md p-4 text-red-800',
      children: [
  ...
  private updateContainerTheme(shadowRoot: ShadowRoot, theme: RaycastTheme): void {
    // Update CSS custom properties
    const host = shadowRoot.host as HTMLElement;
    Object.entries(this.getThemeVariables(theme)).forEach(([property, value]) => {
  ...
  private enhanceClassNames(existingClasses: string = '', theme: RaycastTheme): string {
    const themeClass = this.getThemeClass();
    const enhancedClasses = `${existingClasses} raycast-enhanced ${themeClass}`.trim();
