...
// Validate permissions during definition
  if (meta.permissions && meta.permissions.length > 0) {
      permissionManager.validatePluginPermissions(meta.permissions, meta.id)
      .catch(error => {
        console.error(`Plugin "${meta.id}" permission validation failed:`, error);
...
return {
    ...
    this.exitCallbacks.forEach((callback) => {
        try {
          ...
          console.error(`Error in exit callback for plugin "${this.pluginId}":`, error);
      }
      });
...
if (typeof callback !== 'function') {
    throw new PluginError('onExit callback must be a function', 'INVALID_CALLBACK', this.pluginId);
    }
...
// Utility functions with error handling
  ...
  try {
      if (typeof command !== 'string' || command.trim() === '') {
        throw new PluginError('Command must be a non-empty string', 'INVALID_COMMAND', this.pluginId);
      }
  ...
  showHUD(text: string) {
    if (typeof text !== 'string') {
      console.warn(`Plugin "${this.pluginId}": showHUD expects a string, got ${typeof text}`);
      text = String(text);
    }
    ...
    window.dispatchEvent(new CustomEvent('plugin:hud', {
  ...
  setWindowMode(mode: 'normal' | 'fullscreen' | 'floating') {
    if (!this.context?.window.setMode) {
      console.warn(`Plugin "${this.pluginId}": Window management not available`);
      return;
    }
    ...
    try {
      ...
      console.error(`Plugin "${this.pluginId}" window mode change failed:`, error);
      throw error;
    }
  }
